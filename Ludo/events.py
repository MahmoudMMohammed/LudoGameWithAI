...
class Events:
    ...
    def check_mouse_main_menu_event(self, event_type):
        if event_type == pygame.MOUSEMOTION and self.game.menu.is_main_menu:
            ...
            # Check if mouse is over "Start Game" button
            ...
            # Check if mouse is over "Play VS AI" button
            if self.game.menu.play_vs_ai.collidepoint(self.mouse_pos):
                self.game.menu.play_vs_ai_color = self.settings.DARK_RED
            else:
                self.game.menu.play_vs_ai_color = self.settings.WHITE
            ...
            # Check if "Play VS AI" button is clicked
            if state_of_buttons[0] == 1 and self.game.menu.play_vs_ai.collidepoint(self.mouse_pos):
                self.game.menu.is_main_menu = False
                self.game.start_ai_game()  # Start AI game
    ...
    def check_mouse_third_menu_event(self, event_type):
        if event_type == pygame.MOUSEMOTION and self.game.menu.is_3rd_menu:
            ...
            if self.game.menu.red_rect.collidepoint(self.mouse_pos) and self.game.menu.selected_red:
                self.game.menu.red_surface.set_alpha(100)
                self.game.menu.player_text = f"Select red for player {self.game.player.current_player}"

            elif self.game.menu.selected_red:
                self.game.menu.red_surface.set_alpha(255)

            if self.game.menu.green_rect.collidepoint(self.mouse_pos) and self.game.menu.selected_green:
                self.game.menu.green_surface.set_alpha(100)
                self.game.menu.player_text = f"Select green for player {self.game.player.current_player}"

            elif self.game.menu.selected_green:
                self.game.menu.green_surface.set_alpha(255)

            if self.game.menu.blue_rect.collidepoint(self.mouse_pos) and self.game.menu.selected_blue:

                self.game.menu.blue_surface.set_alpha(100)
                self.game.menu.player_text = f"Select blue for player {self.game.player.current_player}"

            elif self.game.menu.selected_blue:
                self.game.menu.blue_surface.set_alpha(255)

            if self.game.menu.yellow_rect.collidepoint(self.mouse_pos) and self.game.menu.selected_yellow:

                self.game.menu.yellow_surface.set_alpha(100)
                self.game.menu.player_text = f"Select yellow for player {self.game.player.current_player}"

            elif self.game.menu.selected_yellow:
                self.game.menu.yellow_surface.set_alpha(255)
            ...
            if state_of_buttons[0] == 1 and self.game.menu.red_rect.collidepoint(
                    self.mouse_pos) and self.game.menu.selected_red:
                self.game.menu.red_surface.set_alpha(0)
            ...
            if state_of_buttons[0] == 1 and self.game.menu.green_rect.collidepoint(
                    self.mouse_pos) and self.game.menu.selected_green:
                self.game.menu.green_surface.set_alpha(0)
                self.game.player.color_for_player[self.game.menu.counter] = "green"
                self.game.player.current_player += 1
                self.game.menu.counter += 1
                self.game.menu.selected_green = False

            if state_of_buttons[0] == 1 and self.game.menu.blue_rect.collidepoint(
                    self.mouse_pos) and self.game.menu.selected_blue:
                self.game.menu.blue_surface.set_alpha(0)
                self.game.player.color_for_player[self.game.menu.counter] = "blue"
                self.game.player.current_player += 1
                self.game.menu.counter += 1
                self.game.menu.selected_blue = False

            if state_of_buttons[0] == 1 and self.game.menu.yellow_rect.collidepoint(
                    self.mouse_pos) and self.game.menu.selected_yellow:
                self.game.menu.yellow_surface.set_alpha(0)
    ...
    def check_mouse_board_menu_event(self, event_type):
        ...
        if event_type == pygame.MOUSEMOTION and self.game.menu.is_board_menu:
            ...
            if state_of_buttons[0] == 1 and self.game.menu.rtd_button.collidepoint(
                    self.mouse_pos) and not self.game.menu.is_turn_skip:
            ...
            if self.ludo_token.rect.collidepoint(self.mouse_pos) and state_of_buttons[0] == 1:
                ...
                if self.ludo_token.rect.x == self.placeholder_sprite.rect.x and self.ludo_token.rect.y == self.placeholder_sprite.rect.y:
                    ...
                    if self.game.dice.dice_val < 6:
                        # Display the message but do not end the turn
                        ...
                        self.game.dice.current_turn = False  # End the turn after moving the token
                ...
                # This else statement controls the movement for the tiles on the path (movement and winning tiles)
                    ...
                    token_checker += 1
                    self.game.dice.current_turn = False  # End the turn after a valid move
    ...
    def highlight_token_on_board_mouse_event(self):
        ...
        for token_selector, ludo_token in enumerate(self.game.player.current_player_token_group):
            ...
            if not (ludo_token.rect.x == placeholder_sprite.rect.x and ludo_token.rect.y == placeholder_sprite.rect.y):
                ...
                if ludo_token.rect.collidepoint(
                        self.mouse_pos):  # the mouse is hovering over the ludo token that is on the board and not in the base

                    current_token_position = \
                        self.game.player.token_movement_counter[self.game.player.current_player_color][
                            token_selector] + 1  # we take the current token position and to exclude it, we add 1 to it
                    ...
                    if movement_path_indices != 0 and winning_path_indices != 0:  # This situation arises if a dice val arrives such that the tiles to move on include both the movement as well as the winning path tiles
                        ...
                        movement_path_indices_of_tiles_to_highlight = self.game.player.team_path[
                                                                          self.game.player.current_player_color][
                                                                      current_token_position:current_token_position + movement_path_indices]
                        winning_path_indices_of_tiles_to_highlight = self.game.player.winning_path[
                                                                         self.game.player.current_player_color][
                                                                     :winning_path_indices]

                        self.game.board.path_highlight_sprites = [self.game.board.movement_path_sprites[indice] for
                                                                  indice in movement_path_indices_of_tiles_to_highlight]
                        self.game.board.path_highlight_sprites += [
                            self.game.board.winning_path_dict[self.game.player.current_player_color][indice] for indice
                            in winning_path_indices_of_tiles_to_highlight]
                        ...
                        movement_path_indices_of_tiles_to_highlight = self.game.player.team_path[
                                                                          self.game.player.current_player_color][
                                                                      current_token_position:move_val]
                        ...
                        winning_move_val = (
                                                   current_token_position + self.game.dice.dice_val) - self.settings.total_movement_steps

                        if not winning_move_val > (
                                self.settings.winning_path_threshold - self.settings.total_movement_steps + 1):  # The 1 added to the winning path threshold is just to ensure the last token gets highlighted given the dice value isnt too large
                            winning_path_indices_of_tiles_to_highlight = self.game.player.winning_path[
                                                                             self.game.player.current_player_color][
                                                                         current_winning_path_position:winning_move_val]
                            self.game.board.path_highlight_sprites = [
                                self.game.board.winning_path_dict[self.game.player.current_player_color][indice] for
                                indice in winning_path_indices_of_tiles_to_highlight]
                        else:
